...
export function generateSigningMessage(bytes: Uint8Array, domainSeparator: string): Uint8Array {
  ...
  if (!domainSeparator.startsWith("ENDLESS::")) {
    throw new Error(`Domain separator needs to start with 'ENDLESS::'.  Provided - ${domainSeparator}`);
  }
...
export function generateSigningMessageForSerializable(serializable: Serializable): Uint8Array {
  return generateSigningMessage(serializable.bcsToBytes(), `ENDLESS::${serializable.constructor.name}`);
}
