...
export class AccountAddress extends Serializable implements TransactionArgument {
  ...
  static fromString(input: string): AccountAddress {
    ...
    if (input.startsWith("0x")) {
      ...
      // try to decode as bs58
      try {
        const addressBytes = bs58.decode(input);
        return new AccountAddress(addressBytes);
      } catch (error: any) {
        // If the address is not a bs58 encoded address, then it must be a special address.
      }
    }
    ...
    if (parsedInput.length > 64) {
      throw new ParsingError(
        `Hex string is too long, must be 1 to 64 chars long, excluding the leading 0x. => ${parsedInput}`,
