 ...
 */
export const ENDLESS_HARDENED_REGEX = /^m\/44'\/637'\/[0-9]+'\/[0-9]+'\/[0-9]+'?$/;
export const ENDLESS_BIP44_REGEX = /^m\/44'\/637'\/[0-9]+'\/[0-9]+\/[0-9]+$/;
...
export function isValidBIP44Path(path: string): boolean {
  return ENDLESS_BIP44_REGEX.test(path);
}
...
export function isValidHardenedPath(path: string): boolean {
  return ENDLESS_HARDENED_REGEX.test(path);
}
