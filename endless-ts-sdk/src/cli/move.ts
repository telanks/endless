...
export class Move {
  ...
  async init(args: { network?: Network; profile?: string }): Promise<boolean> {
    ...
    const cliArgs = ["endless", "init", `--network=${network ?? "local"}`, `--profile=${profile ?? "default"}`];
  ...
  async compile(args: {
    ...
    const cliArgs = ["endless", "move", "compile", "--package-dir", packageDirectoryPath, "--skip-fetch-latest-git-deps"];
  ...
  async test(args: { packageDirectoryPath: string; namedAddresses: Record<string, AccountAddress> }): Promise<boolean> {
    const { packageDirectoryPath, namedAddresses } = args;
    const cliArgs = ["endless", "move", "test", "--package-dir", packageDirectoryPath, "--skip-fetch-latest-git-deps"];
